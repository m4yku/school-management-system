  ...
  return (
    <div className="flex h-screen bg-slate-50 relative font-sans overflow-hidden z-0 lg:p-0 lg:gap-0">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden bg-slate-50">
        ...
        {/* Center-Left Blob (Slightly different timing) */}
        ...
        {/* Stardust Overlay - Reduced Opacity for better UX */}
      ...
      {/* SIDEBAR */}
      <aside className={`
        fixed z-[70] bg-white/70 backdrop-blur-2xl text-slate-700 flex flex-col transition-all duration-300 ease-in-out shadow-2xl
        left-0 top-0 border border-white/60 m-4 rounded-[2rem] h-[calc(100vh-2rem)]
        ${isSidebarOpen ? 'translate-x-0' : '-translate-x-[120%]'} 
        lg:translate-x-0 lg:static lg:m-0 lg:h-full lg:rounded-none lg:rounded-r-[2rem] lg:shadow-[4px_0_24px_rgba(0,0,0,0.05)] lg:bg-white/50
        w-[calc(100%-2rem)] max-w-xs ${isCollapsed ? 'lg:w-[5.5rem]' : 'lg:w-64'} 
      `}>
        <div className={`h-24 px-6 border-b border-white/60 flex items-center shrink-0 transition-all justify-between ${isCollapsed ? 'lg:justify-center px-0' : ''}`}>
          ...
          <button className="lg:hidden text-slate-500 hover:text-slate-800 p-2 bg-white/50 rounded-xl border border-white/40" onClick={() => setIsSidebarOpen(false)}><X size={24} /></button>
        </div>
            ...
            return (
              <Link 
                ...
                className={`flex items-center p-3.5 rounded-2xl transition-all duration-200 group relative gap-4
                  ${isActive ? 'text-white shadow-md border border-white/40' : 'text-slate-500 hover:bg-white/60 hover:text-slate-800'}
                  ${isCollapsed ? 'lg:justify-center lg:gap-0' : ''}
                `}
                style={isActive ? { backgroundColor: themeColor } : {}}
              >
                <span className={`${isActive ? 'text-white' : 'text-slate-400 group-hover:text-slate-700'} shrink-0 transition-transform group-hover:scale-110`}>{item.icon}</span>
        ...
        <div className="p-4 border-t border-white/60 bg-white/40 shrink-0 rounded-b-[2rem] lg:rounded-b-none lg:rounded-br-[2rem]">
          ...
          <button 
            ...
            className={`flex items-center p-3 mb-2 rounded-2xl text-slate-500 hover:bg-emerald-50 hover:text-emerald-600 transition-all duration-200 group w-full gap-3 ${isCollapsed ? 'lg:justify-center' : ''}`}
            title="Refresh Page"
          >
          ...
          <button onClick={logout} className={`flex items-center p-3 rounded-2xl hover:bg-red-50 text-slate-500 hover:text-red-500 transition-all duration-200 group w-full gap-3 ${isCollapsed ? 'lg:justify-center' : ''}`}>
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full overflow-hidden relative z-10 lg:ml-4">
        <header className="h-24 bg-white/60 backdrop-blur-xl border-b lg:border border-white/80 lg:rounded-none lg:rounded-bl-[2.5rem] rounded-b-3xl flex items-center justify-between px-6 lg:px-10 shadow-sm transition-all">
          <div className="flex items-center space-x-4">
            <button className="p-2.5 rounded-xl bg-white/80 backdrop-blur-sm text-slate-600 lg:hidden hover:bg-white border border-white shadow-sm" onClick={() => setIsSidebarOpen(true)}>
              <Menu size={20} />
            </button>
          ...
          <div className="flex items-center space-x-4 sm:space-x-8">
            <Link to="/teacher/announcements" className={`p-3 rounded-2xl transition-all duration-200 relative ${location.pathname === '/teacher/announcements' ? 'bg-white shadow-md border border-white' : 'text-slate-500 hover:bg-white hover:text-slate-800 hover:shadow-sm border border-transparent'}`} style={location.pathname === '/teacher/announcements' ? { color: themeColor } : {}}>
              <BellDot size={22} />
            ...
            <div className="hidden sm:block w-px h-10 bg-white border-l border-slate-200/50"></div>
        ...
        <div className="p-4 lg:p-8 flex-1 min-h-0 flex flex-col">
          <div className="max-w-7xl mx-auto h-full w-full overflow-y-auto lg:overflow-hidden custom-scroll lg:pr-3">
            <Outlet />
          </div>
        </div>
      </main>
