...
const StudentBilling = () => {
  ...
  const [markAsEnrolled, setMarkAsEnrolled] = useState(true); // Naka-default na true

  const handlePostPayment = async () => {
    // Baguhin ang check: billingData.summary na dapat
    if (!billingData || !billingData.summary) {
      alert(
        "Error: Student data is missing. Please search for the student again.",
      );
      return;
    }
    ...
    try {
      const payload = {
        // PALITAN MO ITO: Mula .student naging .summary
      ...
      if (res.data.status === "success") {
        ...
        setShowSuccess(true);
        // I-set ang receipt info para lumabas yung resibo sa success screen
        setReceiptInfo(res.data.receipt || null);
        setAllocations({});
        setSelectedSch(null);
      } else {
        alert(res.data.message || "Error processing payment");
      }
    } catch (err) {
  ...
  return (
    <div className="p-6 space-y-6 text-left max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 font-medium text-sm italic">
            Search student to manage accounts.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={loading}
            className="bg-blue-600 text-white px-8 py-3.5 rounded-2xl font-black uppercase text-[10px] hover:bg-slate-900 transition-all"
          >
            {loading ? "..." : "Search"}
          </button>
        </div>
      ...
      {!billingData && !loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-32 border-4 border-dashed border-slate-100 rounded-[3.5rem] bg-slate-50/50">
          ...
          <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-2 max-w-xs text-center leading-relaxed">
            Search a Student ID to manage billing, <br /> apply grants, and
            process payments.
          </p>
        </div>
      ) : (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 flex items-center justify-between">
                ...
                <span className="px-4 py-2 bg-emerald-50 text-emerald-600 rounded-full font-black text-[10px] uppercase">
                  {billingData.summary.payment_status || "Active"}
                </span>
              </div>
              ...
              <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden p-8">
                <h3 className="text-xs font-black uppercase text-slate-400 mb-6 flex items-center gap-2">
                  <Receipt size={16} /> Breakdown of Contributions
                </h3>
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {billingData.items.map((item) => {
                      const currentItemBalance =
                        parseFloat(item.amount) - parseFloat(item.paid_amount);
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-4 text-right text-xs font-black">
                            ₱{currentItemBalance.toLocaleString()}
                          </td>
                          ...
                          <td className="py-4 text-right">
                            <button
                              onClick={() =>
                                handlePayFull(item.id, currentItemBalance)
                              }
              ...
              <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8">
                ...
                <div className="space-y-4">
                  {billingData.recent_payments?.length > 0 ? (
                    billingData.recent_payments.map((p, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-center text-xs border-b border-slate-50 pb-4"
                      >
                    ...
                    <p className="text-center italic text-slate-300 text-xs py-4">
            ...
            <div className="space-y-6">
              <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden">
                <p className="text-slate-400 text-[10px] font-black uppercase mb-2 relative z-10">
                  Overall Balance
                </p>
                <h2 className="text-4xl font-black relative z-10">
                  ₱{parseFloat(billingData.summary.balance).toLocaleString()}
                </h2>
                <Wallet
              ...
              <div className="bg-white rounded-[2.5rem] border border-blue-100 p-6 shadow-sm">
                <h3 className="text-[10px] font-black text-blue-600 uppercase mb-4 flex items-center gap-2">
                  <Award size={14} /> Available Scholarships
                </h3>
                <div className="space-y-3">
                  {availableScholarships.length > 0 ? (
                    availableScholarships.map((sch, i) => (
                      ...
                      >
                        <p className="font-black text-slate-800 text-[10px] uppercase leading-tight">
                          {sch.scholarship_name}
                        </p>
                        <p className="text-[10px] font-bold text-blue-600 mb-3">
                          {sch.discount_type === "Percentage"
                            ? `${sch.value}% Discount`
                            : `₱${Number(sch.value).toLocaleString()} Fixed Grant`}
                        </p>
                        <button
                          onClick={() => setSelectedSch(sch)}
                          className="w-full py-2 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase hover:bg-slate-900 transition-all"
                        >
                          Apply Grant
                        </button>
                      </div>
                    ...
                    <p className="text-center italic text-slate-300 text-[10px] uppercase py-4">
                      No Approved Grants
                    </p>
                  )}
                </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[150] flex items-center justify-center p-4 text-left">
          <div className="bg-white rounded-[3rem] w-full max-w-md shadow-2xl p-10 animate-in zoom-in duration-200">
            <h2 className="text-xl font-black text-slate-800 uppercase mb-6 flex items-center gap-2">
              <Banknote size={24} /> Confirm Payment
            </h2>
              ...
              {Object.entries(allocations).map(
                ([id, amt]) =>
                  amt > 0 && (
                    <div
                      key={id}
                      className="flex justify-between text-xs font-bold text-slate-600 uppercase"
                    >
                      <span>
                        {billingData.items.find((i) => i.id == id)?.item_name}
                      </span>
            ...
            <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100 mb-6 flex items-center gap-3">
              ...
              />
              <label
                htmlFor="markEnrolled"
                className="text-[11px] font-black uppercase text-emerald-700 cursor-pointer select-none"
              >
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <button
                onClick={handlePostPayment} // Dito tinatawag yung function sa itaas
                disabled={loading}
                className="w-full bg-emerald-600 text-white py-5 rounded-[2rem] font-black uppercase text-[10px] shadow-xl hover:bg-slate-900 transition-all disabled:opacity-50"
              >
      ...
      {selectedSch &&
        (() => {
          const tuitionItem = billingData.items.find((i) =>
            i.item_name.toLowerCase().includes("tuition"),
          );
          const currentTuitionBal = tuitionItem
          ...
          return (
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[150] flex items-center justify-center p-4 text-left">
              <div className="bg-white rounded-[3rem] w-full max-w-md shadow-2xl p-10">
                <h2 className="text-xl font-black text-slate-800 uppercase mb-2 flex items-center gap-2">
                  <Award size={24} /> Grant Preview
                </h2>
                <p className="text-slate-400 text-xs font-bold uppercase mb-6 tracking-widest">
                  {selectedSch.scholarship_name}
                </p>
                <div className="space-y-3 bg-slate-50 p-6 rounded-3xl mb-8 border border-slate-100">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <button
                    ...
                    className="bg-blue-600 text-white py-4 rounded-2xl text-[10px] font-black uppercase shadow-lg hover:bg-blue-700 transition-all"
                  >
                    Confirm Apply
                  </button>
                </div>
        ...
        <div className="fixed inset-0 bg-blue-900/40 backdrop-blur-md z-[160] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3.5rem] w-full max-w-md p-12 text-center border-t-8 border-emerald-500 shadow-2xl">
            ...
            <h2 className="text-2xl font-black text-slate-800 uppercase leading-tight">
              {receiptInfo?.type === "Scholarship"
                ? "Grant Applied!"
                : "Transaction Successful!"}
            </h2>
            <p className="text-slate-400 font-medium text-xs mt-2 italic uppercase">
              The student's account has been updated.
            </p>

            {/* Breakdown preview sa success modal */}
            {receiptInfo?.type === "Scholarship" &&
              receiptInfo.applied_items?.length > 0 && (
                <div className="mt-6 p-4 bg-slate-50 rounded-2xl border border-slate-100 text-left">
                  ...
                  {receiptInfo.applied_items.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between text-[10px] font-bold text-slate-600 py-1"
                    >
            ...
            <div className="mt-8 space-y-3">
              <button
                ...
                className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 shadow-lg hover:bg-slate-900 transition-all"
              >
              ...
              <button
                ...
                className="w-full text-slate-400 py-3 font-black uppercase text-[10px] hover:text-slate-600 transition-colors"
              >
                Close and Refresh
              </button>
            </div>
