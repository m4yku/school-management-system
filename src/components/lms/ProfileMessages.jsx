...
const ProfileMessages = () => {
  ...
  return (
    <div className="h-[calc(100vh-140px)] min-h-[500px] bg-white md:rounded-[2.5rem] rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex relative animate-in zoom-in-95 duration-500 font-sans">
      
      {/* 1. LEFT SIDEBAR (Hidden on mobile if a contact is selected) */}
      <div className={`border-r border-slate-100 flex-col bg-white shrink-0 transition-all duration-300 ${activeContact ? 'hidden md:flex' : 'flex'} w-full md:w-80`}>
      ...
      <div className={`flex-1 flex-col bg-slate-50/40 relative ${!activeContact ? 'hidden md:flex' : 'flex'} w-full`}>
        {activeContact ? (
          <>
            {/* Header */}
            <div className="p-4 md:p-6 bg-white border-b border-slate-100 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-3 md:gap-4">
                 ...
                 <div className="w-10 h-10 md:w-11 md:h-11 rounded-full bg-blue-600 text-white flex items-center justify-center font-black text-sm border-2 border-blue-50 shrink-0">
                   {getInitials(activeContact.contact_name)}
                 </div>
                 <div className="min-w-0">
                    <h3 className="text-sm font-black text-slate-800 truncate">{activeContact.contact_name}</h3>
              ...
              <div className="flex gap-0 md:gap-1 shrink-0">
                <button className="hidden sm:block p-2.5 text-slate-400 hover:bg-slate-50 hover:text-blue-600 rounded-xl transition-all"><Phone size={18} strokeWidth={2.5} /></button>
                <button className="hidden sm:block p-2.5 text-slate-400 hover:bg-slate-50 hover:text-blue-600 rounded-xl transition-all"><Video size={18} strokeWidth={2.5} /></button>
                
                {/* INFO BUTTON */}
                <button 
                  onClick={() => setShowInfoPanel(!showInfoPanel)}
                  className={`p-2.5 rounded-xl transition-all ${showInfoPanel ? 'bg-blue-50 text-blue-600' : 'text-slate-400 hover:bg-slate-50 hover:text-blue-600'}`}
                >
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-4 md:space-y-6">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.is_you ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] md:max-w-[70%] p-3.5 md:p-4 shadow-sm ${m.is_you ? 'bg-blue-600 text-white rounded-2xl rounded-tr-none' : 'bg-white text-slate-600 border border-slate-100 rounded-2xl rounded-tl-none'}`}>
            ...
            <div className="p-4 md:p-6 bg-white border-t border-slate-100 shrink-0">
               {userRole === 'student' && ['teacher', 'registrar', 'cashier', 'admin'].includes(activeContact.contact_role) && (
                 <div className="mb-3 px-3 py-2 md:px-4 bg-amber-50 border border-amber-100 rounded-xl flex items-center gap-2 animate-in fade-in duration-300">
                   <div className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse shrink-0"></div>
                   <p className="text-[9px] md:text-[10px] font-black text-amber-600 uppercase tracking-widest leading-tight">
                     Note: Daily message limit applies for staff.
                   </p>
                 </div>
               )}

               <div className="bg-slate-50 border border-slate-100 rounded-[1.5rem] p-1.5 md:p-2 flex items-center gap-1 md:gap-2 focus-within:ring-4 ring-blue-500/5 transition-all">
                  <button className="hidden sm:block p-2 md:p-3 text-slate-400 hover:text-blue-600 transition-colors"><Paperclip size={20} strokeWidth={2.5} /></button>
                  <input 
                    type="text" 
                    value={newMessage}
                    ...
                    className="flex-1 bg-transparent border-none outline-none text-sm font-bold text-slate-700 px-3 md:px-2" 
                  />
                  <button 
                    onClick={handleSendMessage}
                    disabled={!newMessage.trim()}
                    className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-blue-600 text-white flex items-center justify-center shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all active:scale-90 disabled:opacity-50 shrink-0"
                  >
      ...
      {activeContact && showInfoPanel && (
        <div className="absolute md:relative inset-0 md:inset-auto z-20 md:z-auto border-l border-slate-100 bg-white flex flex-col shrink-0 animate-in slide-in-from-right duration-300 w-full md:w-72">
          
          {/* Mobile Close Button inside Info Panel */}
          <button 
            onClick={() => setShowInfoPanel(false)} 
            className="md:hidden absolute top-4 right-4 p-2 bg-slate-50 text-slate-400 hover:bg-slate-100 rounded-full transition-colors z-10"
          >
            <X size={18} strokeWidth={3} />
          </button>

          <div className="p-8 text-center border-b border-slate-50 pt-12 md:pt-8">
            <div className="w-20 h-20 rounded-[1.5rem] bg-blue-100 mx-auto mb-4 flex items-center justify-center text-2xl font-black text-blue-600 shadow-inner">
               {getInitials(activeContact.contact_name)}
            </div>
            <h3 className="text-sm font-black text-slate-800 uppercase tracking-tight">{activeContact.contact_name}</h3>
            <p className="text-[10px] font-black text-blue-500 mt-1 uppercase tracking-widest">{activeContact.contact_role}</p>
          </div>

          <div className="p-4 space-y-1">
            <button className="w-full flex items-center justify-between p-3 hover:bg-slate-50 rounded-2xl transition-all group">
              <div className="flex items-center gap-3">
                <BellOff size={16} className="text-slate-400 group-hover:text-blue-600" />
                <span className="text-xs font-bold text-slate-600">Mute Notifications</span>
              </div>
            ...
            {userRole === 'student' && ['teacher', 'registrar', 'cashier', 'admin'].includes(activeContact.contact_role) && (
              <div className="mt-4 p-4 bg-slate-50 rounded-[1.5rem] border border-slate-100">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Chat Policy</span>
                </div>
                ...
                <div className="mt-3 pt-3 border-t border-slate-200/50 flex justify-between items-center">
                  ...
                  <span className="text-[9px] font-black text-emerald-500 uppercase bg-emerald-50 px-2 py-0.5 rounded-full">Active</span>
                </div>
              </div>
            )}
          </div>

          <div className="flex-1 overflow-y-auto px-4 py-2 space-y-2 pb-6">
            <h4 className="px-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Shared Content</h4>
            <button className="w-full flex items-center gap-3 p-3 hover:bg-slate-50 rounded-2xl transition-all">
              <div className="w-8 h-8 rounded-xl bg-amber-50 flex items-center justify-center text-amber-500"><ImageIcon size={16} /></div>
              <span className="text-xs font-bold text-slate-600">Media</span>
              <span className="ml-auto text-[10px] font-black text-slate-300">0</span>
            </button>
            <button className="w-full flex items-center gap-3 p-3 hover:bg-slate-50 rounded-2xl transition-all">
              <div className="w-8 h-8 rounded-xl bg-emerald-50 flex items-center justify-center text-emerald-500"><FileText size={16} /></div>
              <span className="text-xs font-bold text-slate-600">Documents</span>
              <span className="ml-auto text-[10px] font-black text-slate-300">0</span>
            </button>
            <button className="w-full flex items-center gap-3 p-3 hover:bg-slate-50 rounded-2xl transition-all">
              <div className="w-8 h-8 rounded-xl bg-blue-50 flex items-center justify-center text-blue-500"><LinkIcon size={16} /></div>
              <span className="text-xs font-bold text-slate-600">Links</span>
              <span className="ml-auto text-[10px] font-black text-slate-300">0</span>
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center">
              <h3 className="font-black text-slate-800 uppercase tracking-widest text-xs ml-2">New Message</h3>
            ...
            <div className="max-h-[60vh] overflow-y-auto p-4 space-y-2">
              {availableContacts.map((c, i) => (
                <div key={i} onClick={() => startNewChat(c)} className="flex items-center gap-4 p-3.5 hover:bg-blue-50 rounded-[1.5rem] cursor-pointer transition-all border border-transparent hover:border-blue-100 group">
                  <div className="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center font-black text-blue-600 text-xs group-hover:scale-110 transition-transform shrink-0">
                    {getInitials(c.contact_name)}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-black text-slate-800 truncate">{c.contact_name}</p>
